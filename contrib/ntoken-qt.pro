...
contains(NTOKEN_NEED_QT_PLUGINS, 1) {
    DEFINES += NTOKEN_NEED_QT_PLUGINS
...
# also add new translations to src/qt/ntoken.qrc under translations/
TRANSLATIONS = $$files(src/qt/locale/ntoken_*.ts)
...
contains(RELEASE, 1) {
    # Linux: turn ntoken linking back on for c/c++ runtime libraries
    LIBS += -Wl,-Bntoken
}
...
HEADERS += \
    ...
    src/qt/ntokenunits.h \
    src/qt/ntokengui.h \
    src/qt/ntokenamountfield.h \
    src/qt/ntokenaddressvalidator.h \
...
SOURCES += \
    ...
    src/qt/ntoken.cpp \
    src/qt/ntokenaddressvalidator.cpp \
    src/qt/ntokenamountfield.cpp \
    src/qt/ntokengui.cpp \
    src/qt/ntokenstrings.cpp \
    src/qt/ntokenunits.cpp \
    ...
    src/script/ntokenconsensus.cpp \
