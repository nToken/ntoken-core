...
NToken Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9172.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9173 for connections and services them.
...
However, in open source software such as NToken Core, where everyone uses
...
pay attention to for reviewers of NToken Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: NToken Core uses tinyformat, which is type safe. Leave them out to avoid confusion
