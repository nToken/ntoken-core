...
const char * const BITCOIN_CONF_FILENAME = "ntoken.conf";
const char * const BITCOIN_PID_FILENAME = "ntokend.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ntoken" is a composite category enabling all NToken-related debug output
            if(ptrCategory->count(string("ntoken"))) {
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ntoken.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ntoken.conf
