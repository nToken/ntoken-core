...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("NToken Core ntoken-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  ntoken-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded ntoken transaction") + "\n" +
              "  ntoken-tx [options] -create [commands]   " + _("Create hex-encoded ntoken transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded ntoken transaction
